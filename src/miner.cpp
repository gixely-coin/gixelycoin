...
{
    LogPrintf("GixelycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gixelycoin-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running GixelycoinMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("GixelycoinMiner terminated\n");
        throw;
    }
}
