...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("gixelycoin"))
        return false;
...
{
    // Convert gixelycoin:// to gixelycoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("gixelycoin://"))
    {
        uri.replace(0, 12, "gixelycoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Gixelycoin.lnk";
}
...
{
    return GetAutostartDir() / "gixelycoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Gixelycoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  gixelycoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Gixelycoin-Qt"));
